...
export function ImprovedProfessorSignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginResponse.status === 200) {
        ...
        const professorId = loginData.professorId

        localStorage.setItem("userRole", "professor")
        localStorage.setItem("professorId", professorId)

        const professorResponse = await axios.get(`http://localhost:8080/api/professors/${professorId}`, {
          timeout: 10000,
        })
      ...
      <div className="w-full lg:w-3/5 mt-18">
        ...
        <Card className="mt-8 mb-2 mx-auto w-80 max-w-screen-lg lg:w-1/2 p-6">
          ...
          <Typography variant="paragraph" className="text-center text-blue-gray-500 font-medium mt-6">
            Not registered?
            <Link to="/auth/professor/sign-up" className="text-gray-900 ml-1 hover:text-blue-500">
              Create account
            </Link>
          </Typography>
        </Card>
...
export default ImprovedProfessorSignIn
